...
@Controller
public class SearchController_SS {
	...
	public String restaurants(HttpServletRequest request, HttpServletResponse response, Model model) {
		SearchDao dao = sqlSession.getMapper(SearchDao.class);
		model.addAttribute("review",dao.reviewList(request.getParameter("seq")));
...
//						request.setAttribute("cookie_list", dao.cookieListDoa(arr));
