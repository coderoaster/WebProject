...
%>   
	<%int j=0; int k=0;%>
	<form > 		
		<c:forEach items="${mapaddress}" var="address">	
			<% j++; 
	
			%>
			<input type="hidden" value="${address.address}" id="map<%=j%>">	
		</c:forEach>
	</form>	
	<form > 		
		<c:forEach items="${mapname}" var="name">	
			<% k++; 
			%>
			<input type="hidden" value="${name.store_name}" id="name<%=k%>">	
		</c:forEach>
	</form>
			...
			<!-- DB에 존재하지 않는 컬럼존재 -->
					...
					<div class="page">
						<% 
				 		...
				 		%>		
							<a href="search.do?page=<%=i%>&restaurantnum=<%=restaurantnum%>&search=<%=search%>"><%=i%></a>	
						<%
							}
						%> 
					</div>
